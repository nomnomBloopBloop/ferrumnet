...
`check_ack_split_invariance` turns the `bmc` into the prover again: it exhausts **every** ordered way to
split a window of new data into `1..=k` ACKs (down to sub-MSS granularity) and proves the window never grows
past the **bytes acked**, so ACK division buys the attacker nothing. The negative control gives it teeth — a
strawman `PerAckMss` controller (`cwnd += MSS` per ACK, the vulnerable design) is driven through the *same*
enumeration and caught inflating its window on exactly the finely-split traces the attack uses. Two further
receiver attacks frame the boundary (`tcb`'s `misbehaving_receiver_*` test): an ACK **above `SND.NXT`** —
acking data never sent — is dropped by the RFC 793 §3.9 acceptability test (defended); but an **optimistic
ACK** of in-flight-but-undelivered data (≤ `SND.NXT`) is indistinguishable from a genuine ACK, so the TCB
accepts it, advances `SND.UNA`, and frees the undelivered data from the retransmit buffer — the **residual**.
Byte counting and the `SND.NXT` check do not catch a premature-but-in-window ACK; defeating it needs a
*receipt nonce* (RFC 3540-style) the receiver cannot forge for data it never received — characterised here as
the open next step, not built (the honest negative, in the style of the continuum-lift obstruction).
...
| M25 | **Misbehaving-receiver defence** (`bmc::check_ack_split_invariance`; `tcb::misbehaving_receiver_*`) — point the adversary at the protocol *peer*: prove byte counting neutralises **ACK division** by exhausting every way to split a window of new data across ACKs (sub-MSS granularity) and confirming the window never grows past the bytes acked; frame the boundary with the two other receiver attacks | every byte-counting controller (Reno/DCTCP/Learned/Synth) is **proven immune** to ACK division — the window grows with delivered bytes, not ACK count — and a `PerAckMss` strawman (`cwnd += MSS`/ACK) is caught by the *same* enumeration (real teeth). An ACK **above `SND.NXT`** is dropped (RFC 793 §3.9). The **residual**, honestly characterised not built: an **optimistic ACK** of in-flight data is indistinguishable from a genuine one and is accepted (advances `SND.UNA`, frees undelivered data) — defeating it needs a receipt nonce, not byte counting |
